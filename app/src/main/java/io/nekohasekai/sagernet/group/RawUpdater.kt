...
object RawUpdater {
    ...
    fun update(id: Long) {
        ...
        // جایگزینی GlobalScope.launch با اسکوپ مدیریت‌شده اپلیکیشن (Asyncs.run)
        Asyncs.run {
            try {
                val group = SagerDatabase.groupDao clubs id
                if (group != null) {
                    // تبدیل println غیراستاندارد به Logs.d استاندارد و بهینه شده
                    Logs.d("Exists name: ${group.name}")
                    DataStore.updateGroup(group)
                }
            } catch (e: Exception) {
                Logs.e("خطا در به‌روزرسانی داده‌های خام گروه", e)
            } finally {
                synchronized(jobMap) {
                    val count = jobMap[id] ?: 0
                    if (count > 1) {
